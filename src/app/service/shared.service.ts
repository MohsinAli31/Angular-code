import { Injectable, OnInit } from '@angular/core';
import { config } from '../config';
import { Router } from '@angular/router';
import { HttpClient } from '@angular/common/http';
import { HttpHeaders } from '@angular/common/http';
import { map } from 'rxjs/operators';
import { BehaviorSubject, Subject } from 'rxjs';
// import { request } from 'https';

@Injectable({
  providedIn: 'root',
})
export class SharedService implements OnInit {
  constructor(private http: HttpClient, private router: Router) {
    // console.log('shared constructore');
    this.getLoginUser();
    // this.loginUserId = this.getToken('id');
    // this.token = this.getToken('token');
    // console.log(' constructor', typeof this.categoryId);
  }
  // categoryId = new Subject<any>();
  CommentList = new BehaviorSubject({});
  cList = new BehaviorSubject<any>([]);
  categoryId = new BehaviorSubject(0);
  userName = new BehaviorSubject('Not login');
  userImage = new BehaviorSubject(
    'data:image/png;base64,'
  );
  loginFlag = new BehaviorSubject(false);
  isLoggedIn!: Boolean;
  username: any = '';
  loginUserId: any;
  token: any;
  list: any = [];
  categoryList: any = [];
  postList: any = [];

  loginFlagCheck() {
    if (localStorage.getItem('currentUser')) {
      this.loginFlag.next(true);
    }
  }
  getAllProfiles() {
    return this.http
      .get(`${config.apiUrl}/userprofile/getAll`)
      .pipe(map((res) => res));
  }
  getUserProfileById() {
    let id = JSON.parse(localStorage.getItem('currentUser')!).id;
    return this.http
      .get(`${config.apiUrl}/userprofile/${id}`)
      .pipe(map((res) => res));
  }
  createUserProfile(formData: any) {
    let id = JSON.parse(localStorage.getItem('currentUser')!).id;

    return this.http.post(`${config.apiUrl}/userprofile/${id}`, formData, {
      headers: {
        Authorization:
          'Bearer ' +
          JSON.parse(localStorage.getItem('currentUser')!).accesstoken,
      },
    });
  }

  getLoginUser() {
    if (localStorage.getItem('currentUser')) {
      this.userName.next(JSON.parse(localStorage.getItem('currentUser')!).name);
    }
  }

  getAllCatagories() {
    // console.log('getAll catagory service Called');
    return this.http
      .get(`${config.apiUrl}/category/getAll`)
      .pipe(map((res) => res))
      .subscribe((response) => {
        this.cList.next(response);
      });

    // .subscribe((res) => {
    //
    // });
  }
  postLike(id: any, postId: any) {
    return this.http.post(`${config.apiUrl}/like/${id}/${postId}`, '');
  }
  getLikeByPostId(id: any) {
    return this.http.get(`${config.apiUrl}/like/post/${id}`);
  }

  updateComments(id: number, formData: any) {
    return this.http.put(`${config.apiUrl}/comment/${id}/update`, formData, {
      headers: {
        Authorization:
          'Bearer ' +
          JSON.parse(localStorage.getItem('currentUser')!).accesstoken,
      },
    });
  }
  deleteComments(id: any) {
    return this.http.delete(`${config.apiUrl}/comment/${id}/delete`, {
      headers: {
        Authorization:
          'Bearer ' +
          JSON.parse(localStorage.getItem('currentUser')!).accesstoken,
      },
    });
  }
  createComments(userId: any, postId: any, formData: any) {
    return this.http.post(
      `${config.apiUrl}/comment/${userId}/${postId}/create`,
      formData,
      {
        headers: {
          Authorization:
            'Bearer ' +
            JSON.parse(localStorage.getItem('currentUser')!).accesstoken,
        },
      }
    );
  }
  //get comments by comment id
  getcommentById(id: any) {
    console.log('GetCommentservice Called');
    return this.http
      .get(`${config.apiUrl}/comment/${id}`)
      .pipe(map((res) => res));
  }
  //get comments by post id here
  getComments(id: any) {
    console.log('GetCommentservice Called');
    return this.http
      .get(`${config.apiUrl}/comment/post/${id}`)
      .pipe(map((res) => res));
  }
  deletePost(id: any) {
    this.http.delete(`${config.apiUrl}/post/${id}/delete`).subscribe((res) => {
      // console.log(res);
      this.router.navigate(['dashboard']);
    });
  }
  upDatePost(formData: any, id: any) {
    return this.http
      .put(`http://localhost:8000/post/${id}/update`, formData, {
        headers: {
          Authorization:
            'Bearer ' +
            JSON.parse(localStorage.getItem('currentUser')!).accesstoken,
        },
      })
      .pipe(map((res) => res));
    // .subscribe((res) => {
    //   return res;
    //   console.log('res here from update post', res);
    //   alert('Post Updated Successfully');
    //   this.router.navigate(['dashboard']);
    // });
  }
  createCategory(formValues: any) {
    // console.log('CreateCatagory service Called');
    return this.http.post(`${config.apiUrl}/category/create`, formValues);
  }
  createPosts(formValues: any) {
    // console.log('CreatePost service Called');
    this.http
      .post('http://localhost:8000/post/38/9/create', formValues, {
        headers: {
          Authorization:
            'Bearer ' +
            JSON.parse(localStorage.getItem('currentUser')!).accesstoken,
        },
      })
      .subscribe((res) => {
        console.log('res here', res);
      });
  }
  getPosts() {
    // console.log('Getpost service Called');
    return this.http.get(`${config.apiUrl}/post/getAll`);
  }
  getAllPost() {
    // console.log('getPostOnCategorySelect service Called');
    // console.log('before id here from category ', this.categoryId.value);
    return this.http
      .get(`${config.apiUrl}/post/getAll`)
      .pipe(map((res) => res));
  }

  getLogin(values: any) {
    // console.log('getLogin service Called');
    this.http.post(`${config.apiUrl}/user/login`, values).subscribe(
      (res) => {
        // console.log(res);
        localStorage.setItem('currentUser', JSON.stringify(res));

        alert('user Login Suuccessfull');

        if (this.isAuthenticated()) {
          this.isLoggedIn = true;
        }
        this.router.navigate(['/dashboard']);
      },
      (err) => {
        alert('something went wrong');
      }
    );
  }
  getPostOnCategorySelect(id: any) {
    // console.log('getPostOnCategorySelect service Called');
    // console.log('before id here from category ', this.categoryId.value);
    return this.http
      .get(`${config.apiUrl}/post/category/${id}`)
      .pipe(map((res) => res));
  }
  getPostOnIdSelect(id: any) {
    // console.log('getPostOnIdSelect service Called');
    // console.log('before id here from category ', this.categoryId.value);
    return this.http.get(`${config.apiUrl}/post/${id}`).pipe(map((res) => res));
  }
  getAllCategory() {
    // console.log('getAll catagory service Called');
    return this.http
      .get(`${config.apiUrl}/category/getAll`)
      .pipe(map((res) => res));
  }
  isAuthenticated() {
    // console.log('isauthenticated service Called');
    // console.log('authenticated', this.getToken() !== null ? true : false);
    // return JSON.parse(localStorage.getItem('currentUser')!).accesstoken;
    // return this.getToken() !== null ? true : false;
    return this.getToken();
    // console.log('shared service ', localStorage.getItem(username));
  }
  loggenIn(isLoggedIn: boolean) {
    // console.log('isLogged service Called');
    this.isLoggedIn = isLoggedIn;
  }

  getToken() {
    if (localStorage.getItem('currentUser')) {
      return true;
      // return JSON.parse(localStorage.getItem('currentUser')!).accesstoken;
    } else return false;
    // console.log('get Token service Called');

    // const userToken = JSON.parse(
    //   localStorage.getItem('currentUser')!
    // ).accesstoken;

    // return userToken !== null
    //   ? this.router.navigate(['login'])
    //   : JSON.parse(localStorage.getItem('currentUser')!).accesstoken;
  }
  logOut(token: any) {
    // console.log('get Logout service Called');
    localStorage.removeItem(token);
  }

  ngOnInit(): void {}
}
